...
## Instrumentation tool ([download](https://drive.google.com/file/d/15ROYtuPHtEPAD1LsrQyqOdPOKgPBcbjj/view?usp=sharing))
### Requirements
* Java 1.8 or above
...
* `export PATH=$PATH:<path_to_wTest-ICSE22-Submit>/node_modules/.bin` (make sure you have downloaded `wTest-ICSE22-Submit`)
...
* Enter `<path_to_wTest-ICSE22-Submit>` and run `sh wTest.sh STRATEGY PATH_TO_APP_FOLDER TIME_LIMIT ANDROID_SDK_PATH EMULATOR_ID APPIUM_PORT APP_TYPE`. `STRATEGY` can be `wVar` (stands for wTest), or `random` (stands for wDroid), or `api` (stands for wAPI), or `QTesting`, or `ComboDroid`. An example command can be 
`sh wTest.sh wVar <path_to_Instrumentation-ICSE22-Submit>/apps/org.wikipedia 3600 ${ANDROID_HOME} emulator-5554 4723 OPEN_SOURCE`
...
* When reaching the time limit, you can see the output under `<path_to_wTest-ICSE22-Submit>/output/<STRATEGY>/<APP_PACKAGE_NAME>` (e.g., `<path_to_wTest-ICSE22-Submit>/output/wVar/org.wikipedia`). Under this folder, `coverage.txt` contains WebView-specific property coverage and code/method coverage results. In `coverage.txt`, you may see lines starting with `time0` (stands for 10mins), `time1` (stands for 20mins), ..., `time5` (stands for 60mins). If the time limit is set lower than 3600s (60 mins), you may see some lines' content is empty. For example, if the time limit is set as 1200s (20 mins), you will see covered properties at `time0`, `time1`, and `time5` (you will always see results at `time5` no matter what time limit you set because by default the final results will be saved to `time5`).
